...
function OddsRow({ match, size }) {
  ...
  if (!sorted) return (
    <div style={{ display:'flex', gap:5, marginTop:6 }}>
      <span style={{ background:'#f3f4f6', borderRadius:5, padding:'2px 8px', color:TEXT_LITE, fontSize:9, fontWeight:600 }}>Odds TBD</span>
    </div>
  );
  return (
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:2 }}>
      <div style={{ position:'relative' }}>
        {isWinner && <div style={{ position:'absolute', inset:-3, borderRadius:6, border:'2px solid '+teamColor, pointerEvents:'none', zIndex:1 }}/>}
        <input type="number" inputMode="numeric" min="1" max="15" value={value}
          onChange={e=>onChange(e.target.value)}
          style={{ width:40, height:38, background:value!==''?scoreBg(value,par):'#f9fafb', border:'1px solid '+(value!==''?scoreColor(value,par)+'44':BORDER), borderRadius:6, color:value!==''?scoreColor(value,par):'#9ca3af', textAlign:'center', fontSize:15, fontWeight:'bold', outline:'none', fontFamily:'Georgia,serif', padding:0, WebkitAppearance:'none', MozAppearance:'textfield' }}/>
      </div>
      {value!==''&&<div style={{ fontSize:9, color:scoreColor(value,par), lineHeight:1, fontWeight:'bold' }}>{scoreLabel(value,par)}</div>}
    </div>
  );
}

function Strip({ results, holes, height }) {
  const h = height || 6;
  return (
    <div style={{ display:'flex', gap:2, height:h, borderRadius:3, overflow:'hidden' }}>
      {Array(holes).fill(0).map((_,i) => (
        <div key={i} style={{ flex:1, borderRadius:1, background:results[i]==='j'?JACK_COL:results[i]==='o'?JOE_COL:results[i]==='h'?'#cbd5e0':'#e2e8f0' }}/>
      ))}
    </div>
  );
}
...
function MuniCornerAd() {
  return (
    <a href="https://www.instagram.com/mastersofmuni" target="_blank" rel="noopener noreferrer"
      style={{ position:'fixed', bottom:16, right:14, zIndex:99, display:'flex', alignItems:'center', gap:6, background:'rgba(255,255,255,0.95)', border:'1px solid '+BORDER, borderRadius:10, padding:'6px 10px', boxShadow:'0 2px 12px rgba(0,0,0,0.12)', textDecoration:'none' }}>
      <img src="/masters-of-muni.png" alt="Masters of Muni" style={{ height:28, width:'auto', objectFit:'contain' }}/>
      <div>
...
function MatchDetail({ match, scores, onChange, onClose }) {
  ...
  return (
    <div onClick={e=>{if(e.target===e.currentTarget)onClose();}} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:200, overflowY:'auto', WebkitOverflowScrolling:'touch', display:'flex', justifyContent:'center', alignItems:'flex-start', padding:8 }}>
      <div style={{ background:CREAM, borderRadius:16, width:'100%', maxWidth:740, margin:'auto', fontFamily:'Georgia,serif', overflow:'hidden', boxShadow:'0 20px 60px rgba(0,0,0,0.2)' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <div>
              <div style={{ color:'rgba(255,255,255,0.7)', fontSize:9, letterSpacing:3, textTransform:'uppercase', marginBottom:3 }}>
                ...
                {match.tee && <span style={{ marginLeft:6, background:'rgba(255,255,255,0.15)', borderRadius:4, padding:'1px 6px' }}>⏰ {match.tee}</span>}
              </div>
              <div style={{ color:WHITE, fontSize:17, fontWeight:'bold' }}>{match.lb}</div>
            </div>
            <button onClick={onClose} style={{ background:'rgba(255,255,255,0.2)', border:'none', color:WHITE, fontSize:18, cursor:'pointer', borderRadius:6, width:32, height:32, display:'flex', alignItems:'center', justifyContent:'center' }}>✕</button>
          </div>
        </div>
        <div style={{ padding:'16px 14px' }}>
          <div style={{ display:'flex', gap:8, marginBottom:12 }}>
            <div style={{ flex:1, background:JACK_BG, borderRadius:10, padding:'10px 12px', border:'1.5px solid '+JACK_COL+'33' }}>
              <div style={{ color:JACK_COL, fontSize:9, fontWeight:700, letterSpacing:2, textTransform:'uppercase', marginBottom:3 }}>🟩 Team Jack</div>
            ...
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
              <div style={{ background:statusBg, borderRadius:10, padding:'8px 10px', textAlign:'center', border:'1.5px solid '+statusColor+'33' }}>
                <div style={{ fontSize:20, fontWeight:'bold', color:statusColor, lineHeight:1 }}>{state.txt}</div>
                <div style={{ color:TEXT_LITE, fontSize:9, marginTop:2 }}>{state.status==='done'?'FINAL':'thru '+state.holesPlayed}</div>
              </div>
            </div>
            <div style={{ flex:1, background:JOE_BG, borderRadius:10, padding:'10px 12px', border:'1.5px solid '+JOE_COL+'33' }}>
              <div style={{ color:JOE_COL, fontSize:9, fontWeight:700, letterSpacing:2, textTransform:'uppercase', marginBottom:3 }}>🟥 Team Joe</div>
          ...
          {format==='bestball'&&<div style={{ color:TEXT_LITE, fontSize:9, marginTop:7, fontStyle:'italic' }}>Enter each player's score — the lower score counts for match play</div>}
            ...
            <table style={{ borderCollapse:'collapse', fontSize:11, minWidth:course.holes*48+120, width:'100%' }}>
              ...
              <tbody>
                <tr style={{ borderTop:'2px solid '+BORDER, background:'#f7fdf9' }}>
                  <td style={{ padding:'5px 9px', color:JACK_COL, fontWeight:'bold', fontSize:10, borderRight:'1px solid '+BORDER, whiteSpace:'nowrap' }}>🟩 {match.j[0]}{match.j[1]?' / '+match.j[1]:''}</td>
                  {Array(course.holes).fill(0).map((_,i)=>{const s=scores[i]||{j:'',o:''};return<td key={i} style={{ padding:'3px 2px', textAlign:'center' }}><ScoreCell value={s.j} par={course.par[i]} onChange={v=>onChange(i,'j',v)} isWinner={state.holeResults[i]==='j'} teamColor={JACK_COL}/></td>;})}
                ...
                <tr style={{ borderTop:'1px solid '+BORDER, background:'#fdf7f7' }}>
                  <td style={{ padding:'5px 9px', color:JOE_COL, fontWeight:'bold', fontSize:10, borderRight:'1px solid '+BORDER, whiteSpace:'nowrap' }}>🟥 {match.o[0]}{match.o[1]?' / '+match.o[1]:''}</td>
                  {Array(course.holes).fill(0).map((_,i)=>{const s=scores[i]||{j:'',o:''};return<td key={i} style={{ padding:'3px 2px', textAlign:'center' }}><ScoreCell value={s.o} par={course.par[i]} onChange={v=>onChange(i,'o',v)} isWinner={state.holeResults[i]==='o'} teamColor={JOE_COL}/></td>;})}
                  <td style={{ textAlign:'center', borderLeft:'1px solid '+BORDER, padding:'3px 7px' }}>
                ...
                <tr style={{ borderTop:'1px solid '+BORDER, background:'#f9fafb' }}>
                  ...
                  {state.holeResults.map((r,i)=>(<td key={i} style={{ textAlign:'center', padding:'4px 2px', fontSize:12 }}>{r==='j'&&<span style={{ color:JACK_COL, fontWeight:'bold' }}>▲</span>}{r==='o'&&<span style={{ color:JOE_COL, fontWeight:'bold' }}>▲</span>}{r==='h'&&<span style={{ color:'#cbd5e0' }}>–</span>}</td>))}
                  <td style={{ borderLeft:'1px solid '+BORDER }}/>
                </tr>
              </tbody>
            </table>
          ...
          <div style={{ marginTop:16 }}>
            ...
            <div style={{ display:'flex', gap:2, alignItems:'flex-end', height:28 }}>
              {(()=>{let run=0;return state.holeResults.map((r,i)=>{if(r==='j')run++;else if(r==='o')run--;const h=Math.abs(run);const hex=Math.min(30+h*20,99).toString(16).padStart(2,'0');return<div key={i} style={{ flex:1, borderRadius:2, alignSelf:'flex-end', height:h===0?3:Math.min(5+h*5,28), background:run>0?JACK_COL+hex:run<0?JOE_COL+hex:'#e2e8f0' }}/>;});})()}
            </div>
          ...
          <div style={{ display:'flex', gap:10, marginTop:12, flexWrap:'wrap' }}>
            {[['Eagle+','#b7791f'],['Birdie','#2d7a3a'],['Par','#2d3748'],['Bogey','#c0392b'],['Double+','#7b1d1d']].map(([l,c])=>(<div key={l} style={{ display:'flex', alignItems:'center', gap:3 }}><div style={{ width:8, height:8, borderRadius:2, background:c, opacity:0.8 }}/><span style={{ color:TEXT_LITE, fontSize:8 }}>{l}</span></div>))}
          </div>
        </div>
      </div>
    </div>
...
function MatchCard({ match, scores, onClick }) {
  ...
  const statusColor = state.leader==='j'?JACK_COL:state.leader==='o'?JOE_COL:TEXT_MID;
  return (
    <div onClick={onClick} style={{ background:WHITE, border:'1px solid '+(done?GREEN+'55':BORDER), borderRadius:10, padding:'12px 14px', cursor:'pointer', boxShadow:done?'0 2px 8px '+GREEN+'22':'0 1px 3px rgba(0,0,0,0.06)', transition:'all .15s', userSelect:'none', WebkitUserSelect:'none' }} onTouchStart={e=>e.currentTarget.style.background='#f0f4f0'} onTouchEnd={e=>e.currentTarget.style.background=WHITE}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
        <div style={{ flex:1, minWidth:0 }}>
          {match.tee&&<div style={{ color:TEXT_LITE, fontSize:9, marginBottom:5, display:'flex', alignItems:'center', gap:4 }}><span>⏰</span><span>{match.tee}</span></div>}
          <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:4 }}>
            <div style={{ width:10, height:10, borderRadius:2, background:JACK_COL, flexShrink:0 }}/>
            <div style={{ color:JACK_COL, fontSize:12, fontWeight:600, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{match.j.join(' & ')}</div>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
            <div style={{ width:10, height:10, borderRadius:2, background:JOE_COL, flexShrink:0 }}/>
            <div style={{ color:JOE_COL, fontSize:12, fontWeight:600, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{match.o.join(' & ')}</div>
          </div>
          {isSingles&&<OddsRow match={match} size="sm"/>}
        </div>
        <div style={{ textAlign:'right', marginLeft:10, flexShrink:0 }}>
          <div style={{ fontSize:15, fontWeight:'bold', color:statusColor, background:state.leader?(state.leader==='j'?JACK_BG:JOE_BG):'#f7fafc', borderRadius:6, padding:'4px 10px', display:'inline-block' }}>{state.txt}</div>
          <div style={{ color:TEXT_LITE, fontSize:9, marginTop:3 }}>{done?'FINAL':state.holesPlayed>0?'thru '+state.holesPlayed:'tap to score →'}</div>
        </div>
      </div>
...
function Section({ title, subtitle, matches, allScores, onOpen, jackPts, joePts, expanded, onToggle }) {
  return (
    <div style={{ marginBottom:24 }}>
      <div onClick={onToggle} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:'2px solid '+GREEN+'33', paddingBottom:10, marginBottom:12, cursor:'pointer', userSelect:'none' }}>
        <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <div style={{ display:'flex', alignItems:'center', gap:6, background:WHITE, borderRadius:8, padding:'4px 10px', border:'1px solid '+BORDER, boxShadow:'0 1px 3px rgba(0,0,0,0.06)' }}>
      ...
      {expanded&&<div style={{ display:'flex', flexDirection:'column', gap:8 }}>{matches.map(m=><MatchCard key={m.id} match={m} scores={allScores[m.id]||[]} onClick={()=>onOpen(m.id)}/>)}</div>}
    </div>
  );
}
  ...
  useEffect(()=>{
    (async()=>{
      try{
        ...
        data.forEach(({match_id,hole,team,score})=>{if(next[match_id]?.[hole]!==undefined)next[match_id][hole][team]=score===null?'':String(score);});
        setAllScores(next);setSyncStatus('live');
  ...
  useEffect(()=>{
    const ch=supabase.channel('scores_live')
      .on('postgres_changes',{event:'*',schema:'public',table:'scores'},payload=>{
        ...
        if(!match_id)return;
        setAllScores(prev=>{const ms=[...(prev[match_id]||[])];if(!ms[hole])ms[hole]={j:'',o:''};ms[hole]={...ms[hole],[team]:score===null?'':String(score)};return{...prev,[match_id]:ms};});
      })
      .subscribe(s=>{if(s==='SUBSCRIBED')setSyncStatus('live');if(s==='CHANNEL_ERROR')setSyncStatus('error');});
    return()=>supabase.removeChannel(ch);
  },[]);

  const handleChange=useCallback((matchId,holeIdx,team,val)=>{
    setAllScores(prev=>{const updated=(prev[matchId]||[]).map((s,i)=>i===holeIdx?{...s,[team]:val}:s);return{...prev,[matchId]:updated};});
    const key=matchId+'_'+holeIdx+'_'+team;
    clearTimeout(saveTimers.current[key]);
    saveTimers.current[key]=setTimeout(async()=>{
      try{await supabase.from('scores').upsert({match_id:matchId,hole:holeIdx,team,score:val===''?null:Number(val)},{onConflict:'match_id,hole,team'});setSyncStatus('live');}catch{setSyncStatus('error');}
    },500);
  },[]);
